...
"""
### function impulse(sys::StateSpace, t::Vector = []; solver=Tsit5(), plot=true)
...
The impulse response is computed using `lsim` to solve the following dynamic
system:
...
in which x(0) = B[:,i] for the selected ith input.
...
function impulse(sys::StateSpace, t::Vector = []; solver=Tsit5(), plot=true)
    ...
    if num_inputs == 1
        lsim(sys, (t)->0, t, sys.B;
             solver=solver, plot=plot, title="Impulse Response")
    else
        yo = []
        to = []
        uo = []
        ...
        for i = 1:num_inputs
            x0 = sys.B[:,i]
            yi, ti, ui = lsim(sys, (t)->zeros(num_inputs), t, x0;
                              solver=solver, plot=plot, title="Impulse Response - From Input #$i")
            push!(yo,yi)
            push!(to,ti)
            push!(uo,ui)
        end
...
"""
### function impulse(G::TransferFunction, t::Vector = []; solver=Tsit5(), plot=true)
...
function impulse(G::TransferFunction, t::Array = []; solver=Tsit5(), plot=true)
    # Convert the transfer function to state space and call step.
    impulse(tf2ss(G), t; plot=plot, solver=solver)
end
